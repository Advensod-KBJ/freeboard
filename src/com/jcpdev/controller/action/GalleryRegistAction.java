...
public class GalleryRegistAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String path = "D:\\JCP0716\\upload"; // 서버컴퓨터 로컬 경로
		// 프로젝트 폴더와 관련 없는 경로일때 url 매핑을 server.xml에 설정한다.
		// String path = request.getServletContext().getRealPath("/image");
		// 업로드 폴더가 프로젝트 폴더의  하위 경로 image 폴더+ 일때 사용
		GalleryDao gdao = GalleryDao.getInstance();
		int size = 10 * 1024 * 1024;	// 10MB, 최대파일크기
		try {
			
			// 업로드된 파일을 받을 수 있는 MultipeatRequest타입의  request 객체 생성
			MultipartRequest multi_request = new MultipartRequest(request, path, size, "UTF-8",	new DefaultFileRenamePolicy());
			
			// 기존 파일명 중복된 파일명은 이름을 변경-> 기본방식은 파일명뒤에 순차적으로 번호 부여
			...
			// 새로운 파일명 규칙으로 파일명 변경하기도 할수 있음(참고)
